...
<?php 
        ...
        //----------------------------------------------------------------------------
          ...
          if(isset($_POST['valider'])){ 
            ...
            $test=array("launch_zenon.sh","launch_modulo.sh");
            
            if ($outils=='Zenon') {$c=0;$f=0;}
            elseif ($outils=='Zenon Modulo') {$c=1; $f=1;}
			elseif ($outils=='Zenon et Zenon Modulo') {$c=0; $f=1;}
            
            
            $sql_inBenc= 'INSERT INTO benchmark 
			VALUES ("","","$tps_limite","$memoire","",NOW(),"$_SESSION['id_utilisateur']")';
            mysql_query($sql_inBenc) or die ('Erreur SQL !'.$sql_insert.'<br />'.mysql_error());
            
            for ($cnt=$c;$cnt<=$f;$cnt++){
				
					$launch=$test[cnt];
				
            for($a=0; $a<count($problem_selec); $a++){
			...
			$command=$launch+' '+$problem_selec[$a]+' '+$tps_limite+'s '+$memoire+'G'; // variables php que viens d arphi
			`$command`;
			...
			if ($res=="PROOF-FOUND"){$res=1;} else {$res2=0;}
			
			// recuperation id_problème
			 $sql='SELECT id_probleme FROM probleme WHERE nom_probleme='+$problem_selec[$a];
                    $idprob = mysql_query($sql) or die('Erreur SQL !<br />'.$sql.'<br />'.mysql_error()); 
			
			// recuperation dernier id benchmark
			$sql='SELECT max(id_benchmark) FROM benchmark';
				$idBench= mysql_query($sql) or die('Erreur SQL !<br />'.$sql.'<br />'.mysql_error());

			//id outil $cnt---> 0 pour zenon et 1 pour zenon modulo
			
			// enregistrement 
			$sql_insert= 'INSERT INTO test 
			VALUES ("$problem_selec[$a]","$res2","$time","$date","$cnt","$idprob","$idBench")';  
